...
#              BOT CONFIGURATION
# ==========================================
BOT_TOKEN = "8767191402:AAFHLBN8URoTKcOLHZ_zR4JXPSvrBHtedWg"  # Put your @BotFather bot token here
...
MAPPED = "𝘼𝘽𝘾𝘿𝙀𝙁𝙂𝙃𝙄𝙅𝙆𝙇𝙈𝙉𝙊𝙋𝙌𝙍𝙎𝙏𝙐𝙑𝙒𝙓𝙔𝙕𝙖𝙗𝙘𝙙𝙚𝙛𝙜𝙝𝙞𝙟𝙠𝙡𝙢𝙣𝙤𝙥𝙦𝙧𝙨𝙩𝙪𝙫𝙬𝙭𝙮𝙯"
...
bot_state = {}
active_wizards = {}  # Tracks bot onboarding conversations
active_clients = []  # Tracks live Telethon clients running in background
...
async def edit_or_reply(event, text):
    try:
        if getattr(event, 'out', False) or getattr(event, 'is_channel', False):
            ...
            except: return await event.reply(text)
        else:
        ...
        try: return await event.reply(text)
        except: pass
...
✘ `.sudolist`"""
    ...
    async def spm_cmd(event):
        ...
        if not text: return await edit_or_reply(event, f"`{fmt('Provide text to spam', None)}`")
            ...
            async def worker(target_uid=uid):
                while True:
                    try:
                        ...
                        text_to_send = custom_text if custom_text else random.choice(MAFIA_TEXTS)
    ...
    async def ms_cmd(event):
        ...
        chat_id = event.chat_id
        texts_to_use = [t.strip() for t in custom_text.split(',')] if custom_text else MAFIA_TEXTS
    ...
    async def stickerspam_cmd(event):
        if not await is_allowed(event): return
        if not event.is_reply: return await edit_or_reply(event, f"`{fmt('Reply to a sticker to spam it', None)}`")
        reply = await event.get_reply_message()
        if not reply.media or not hasattr(reply.media, 'document'): 
            return await edit_or_reply(event, f"`{fmt('Not a valid sticker', None)}`")
        chat_id = event.chat_id
    ...
    async def reply_on(event):
        if not await is_allowed(event): return
        targets, _ = await get_targets(event)
        if not targets: return await edit_or_reply(event, f"`{fmt('Reply or mention users to lock Mafia Reply', None)}`")
        chat_id = event.chat_id
    ...
    async def purgeto_cmd(event):
        if not await is_allowed(event): return
        if not event.is_reply: return await edit_or_reply(event, f"`{fmt('Reply to a message to set end point!', None)}`")
        chat = event.chat_id
        if chat not in state.get("purgefrom", {}): return await edit_or_reply(event, f"`{fmt('Use .purgefrom first!', None)}`")
        reply = await event.get_reply_message()
    ...
    async def purge_cmd(event):
        if not await is_allowed(event): return
        if not event.is_reply: return await edit_or_reply(event, f"`{fmt('Reply to a message to purge downwards!', None)}`")
    ...
    async def tts_cmd(event):
        ...
        if not text and event.is_reply:
            reply = await event.get_reply_message()
            text = reply.text
        if not text: return await edit_or_reply(event, f"`{fmt('Provide text for TTS!', None)}`")
        ...
        try:
            ...
            await st.edit(f"`{fmt(f'TTS Error: Please pip install gtts', 'stop')}`")
    ...
    async def leave_cmd(event):
        ...
        chat = await event.get_chat()
        try:
            await edit_or_reply(event, f"`{fmt('Leaving this chat. Goodbye!', 'start')}`")
            await client(LeaveChannelRequest(chat))
        except Exception as e:
            await edit_or_reply(event, f"`{fmt(f'Error leaving: {e}', 'stop')}`")
    ...
    async def removebots_cmd(event):
        ...
        await st.edit(f"`{fmt(f'Removed {removed} bots!', 'start')}`")
    ...
    async def passive_handler(event):
        ...
        async def process_passive_actions():
            ...
            if chat in state["reply"] and uid in state["reply"][chat]:
                if not getattr(event, 'is_channel', False): await auto_typing(chat)
                try: sent_msg = await event.reply(random.choice(MAFIA_TEXTS))
                except: sent_msg = await client.send_message(chat, random.choice(MAFIA_TEXTS), reply_to=event.id)
...
async def start_bot_hoster():
    bot = TelegramClient("sessions/mafia_hoster_bot", 373, "c3e7f55f5d9caf131baf")
    await bot.start(bot_token=BOT_TOKEN)
    print("✨ Mafia Hoster Bot is active! Connect with it via Telegram.")

    # Shared UI Markup Elements
    start_buttons = [
        ...
        [types.KeyboardButtonCallback(text="Support / Report Issue ❤️", data=b"support")]
    ]
    ...
    async def bot_start(event):
        ...
        welcome_text = (
            "**Welcome to Mafia Userbot Hoster!**\n"
    ...
    async def bot_callbacks(event):
        ...
        if data == b"host_bot":
            ...
            await event.answer("Go to my.telegram.org, login, and copy your API ID & HASH.", alert=True)
        elif data == b"help":
            await event.answer("This bot deploys an interactive self-managed mafia userbot framework.", alert=True)
        elif data == b"support":
            await event.answer("Contact developers or authorized hosts for support.", alert=True)
    ...
    async def wizard_message_handler(event):
        ...
        if step == "api_id":
            ...
            # Create standard userbot instance dynamically
            ...
            if not await client.is_user_authorized():
                ...
                await bot.send_message(event.chat_id, "✅ **Account already logged in and verified! Initializing functions...**")
            ...
            try:
                ...
                me = await client.get_me()
                await bot.send_message(event.chat_id, f"✅ **Successfully hosted as: {me.first_name}**\nUserbot functions are now fully active! 🚀")
                await register_handlers(client, wizard["session_name"])
                asyncio.create_task(client.run_until_disconnected())
                active_clients.append(client)
                del active_wizards[user_id]
            except SessionPasswordNeededError:
                wizard["step"] = "password"
                await bot.send_message(event.chat_id, "🔒 **Two-Step Verification active. Enter cloud password:**", buttons=cancel_keyboard)
            except Exception as e:
...
async def load_existing_sessions():
    ...
    for s_file in session_files:
        ...
        if session_name == "mafia_hoster_bot": continue  # Skip central manager bot
        session_path = os.path.join(SESSIONS_DIR, session_name)
        
        # We try standard hardcoded credentials first for auto-reconnects, or fallbacks if saved.
        client = TelegramClient(session_path, 373, "c3e7f55f5d9caf131baf")
        try:
            await client.connect()
...
async def main():
    if BOT_TOKEN == "YOUR_TELEGRAM_BOT_TOKEN_HERE":
        print("❌ Error: Set your real BOT_TOKEN at line 44 before starting.")
        return
